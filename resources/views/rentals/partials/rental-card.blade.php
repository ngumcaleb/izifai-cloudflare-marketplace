...
<div class="group relative w-full min-w-0 bg-white overflow-hidden border border-black/[0.07] rounded-xl hover:shadow-[0_18px_44px_-14px_rgba(20,27,11,0.18)] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between">
    ...
    <a href="{{ route('rentals.show', $item->slug) }}" class="block flex-1 flex flex-col justify-between">
        ...
        <div class="relative aspect-square overflow-hidden bg-[#f6f6f6]">
            ...
            <span class="absolute top-3 right-3 z-20 px-2.5 py-1 rounded-md -skew-x-12 bg-[#1c201e] text-[#9acd32] text-[9px] font-extrabold uppercase tracking-[0.12em] shadow-md">
        ...
        <div class="p-3.5 flex flex-col flex-1 justify-between">
            <div>
                ...
                <div class="flex items-center justify-between gap-2">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#9aa19c] truncate">
                ...
                {{-- Rental Title --}}
                <h3 class="text-[13px] font-bold text-[#1c201e] leading-snug line-clamp-2 mt-1.5 group-hover:text-[#7ca81d] transition-colors">
                    {{ $item->name }}
                </h3>
                ...
                <div class="mt-1.5 flex items-center gap-1.5">
                    ...
                    @else
                        <span class="inline-flex items-center gap-1 text-[10px] font-bold text-[#659316]">
            ...
            <div class="flex items-end justify-between gap-2 mt-2.5 pt-2.5 border-t border-[#f0f1f0]">
                <div>
                    <span class="inline-flex items-baseline gap-1 mt-0.5 px-2.5 py-1 rounded-md -skew-x-6 bg-[#f2f9df] text-[#659316]">
                        <span class="skew-x-6 text-[14px] font-black leading-tight tnum">
                            {{ number_format($item->rate) }} <span class="text-[10px] font-bold">F/{{ substr($item->billing_unit ?? 'day', 0, 1) }}</span>
                        </span>
                    </span>
                </div>
