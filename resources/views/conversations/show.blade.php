    ...
    <div class="shrink-0 bg-white border-b border-[#e8eae8] px-2.5 sm:px-4 py-2 sm:py-2.5 flex items-center gap-2 sm:gap-2.5 z-10">
        ...
        <div class="shrink-0 relative" x-data="{ menuOpen: false, deleteOpen: false }" @click.outside="menuOpen = false">
            ...
            <div x-show="menuOpen" x-transition x-cloak
                 class="absolute right-0 top-10 sm:top-11 z-30 w-52 bg-white border border-[#e8eae8] rounded-xl shadow-lg overflow-hidden">
                @if($whatsappNumber)
                <a href="https://wa.me/{{ wa_url($whatsappNumber) }}?text={{ urlencode('Hi ' . $otherUser->name . ', I am chatting with you on Izifai.') }}"
                   target="_blank"
                   class="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-[12px] font-semibold text-[#1c201e] hover:bg-[#f2f9df] transition-all">
                    ...
                    <i class="fa-solid fa-arrow-up-right-from-square text-[11px] ml-auto text-[#9aa19c]" style=""></i>
                </a>
                @endif
                <button type="button" @click.stop="menuOpen = false; deleteOpen = true"
                        class="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-[12px] font-semibold text-[#dc2626] hover:bg-[#fdecec] transition-all rounded-b-xl">
                    ...
                    Delete conversation
                    <span class="ml-auto text-[9px] font-bold uppercase tracking-wide text-[#9aa19c]">permanent</span>
                </button>
            </div>
